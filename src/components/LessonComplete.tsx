...
export function LessonComplete({
  correctCount, totalCount, xpEarned, streak, progress,
  previousBadgeCount, onNextLesson, onNextModule, nextModuleName, onRetry, onBackToModule,
}: LessonCompleteProps) {
  ...
  useEffect(() => {
    ...
    const colors = ["#F5A623", "#FF6B6B", "#4ECDC4", "#A855F7", "#F97316"];
    const frame = () => {
  ...
  return (
    <motion.div
      ...
      className="relative flex flex-col items-center gap-5 rounded-3xl border border-border/60 bg-card p-8 text-center shadow-lg overflow-hidden"
    >
      ...
      <motion.div
        ...
        className="relative flex h-24 w-24 items-center justify-center rounded-full bg-accent/10"
      >
      ...
      <p className="text-5xl font-black text-accent">{pct}%</p>
      <p className="text-muted-foreground font-semibold">{correctCount}/{totalCount} correct</p>
      ...
      {newBadges.length > 0 && (
        <motion.div
          ...
          className="w-full rounded-2xl border border-accent/25 bg-accent/5 p-4"
        >
      ...
      <div className="mt-2 flex w-full flex-col gap-2">
        {onNextLesson && (
          <Button onClick={onNextLesson} className="w-full gap-2 rounded-xl font-bold h-12 text-base bg-accent text-accent-foreground hover:bg-accent/90 shadow-md">
            Next Lesson <ArrowRight className="h-4 w-4" />
          </Button>
        )}
        {!onNextLesson && onNextModule && (
          <Button onClick={onNextModule} className="w-full gap-2 rounded-xl font-bold h-12 text-base bg-accent text-accent-foreground hover:bg-accent/90 shadow-md">
